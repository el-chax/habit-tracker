...
import { 
  Calendar, Check, X, Moon, Sun, ChevronLeft, ChevronRight, Plus,
  Lock, Unlock, PieChart, Activity, TrendingUp, Award, Zap,
  LogOut, User, Loader2, AlertTriangle, Menu, ArrowLeft, Flame, Image as ImageIcon, Upload, HelpCircle, ChevronRight as ChevronRightIcon
} from 'lucide-react';
...
const TUTORIAL_STEPS = [
    {
        ...
        desc: "This isn't just a checklist. It's a system to visualize your discipline. Let's walk through how it works."
    },
    {
        title: "Daily Habits & Rules",
        desc: "Add habits on the Journal page. Checking a box gives you XP. You can freely edit the current month, but past months are PERMANENTLY LOCKED to ensure your history is real."
    },
    {
        title: "Analytics & Rhythm",
        desc: "Your streak only increases if you complete 100% of your habits for the day. Use the Analytics dashboard to track your daily rhythm and consistency score."
    },
    {
        title: "The Vision Board",
        desc: "On your Profile, upload a photo of your goal. It starts blurry. As you gain XP (Level up), it becomes clearer. Reach Level 100 to see it fully."
    },
    {
        title: "The Code of Honor",
        desc: "This system relies on your honesty. You are given the freedom to edit today, but you are doing this for yourself. Don't cheat the streak. Good luck."
    }
];
...
export default function App() {
  ...
  const theme = darkMode 
    ? { bg: 'bg-[#09090b]', text: 'text-zinc-100', textMuted: 'text-zinc-500', card: 'bg-[#121214]', cardLight: 'bg-[#18181b]', border: 'border-zinc-800', tabBorder: 'border-zinc-800', accent: 'text-emerald-400', accentBg: 'bg-emerald-500', hover: 'hover:bg-zinc-800', gridHeader: 'bg-[#09090b]', gridBorder: 'border-zinc-800', checkBg: 'bg-emerald-900/30', checkFill: 'bg-emerald-500', scrollThumb: '#3f3f46', scrollTrack: '#18181b', heatmapEmpty: 'bg-zinc-800/50 border border-zinc-800' }
    : { bg: 'bg-[#fafafa]', text: 'text-zinc-900', textMuted: 'text-zinc-400', card: 'bg-white', cardLight: 'bg-zinc-50', border: 'border-zinc-200', tabBorder: 'border-zinc-200', accent: 'text-emerald-700', accentBg: 'bg-emerald-600', hover: 'hover:bg-zinc-50', gridHeader: 'bg-[#fafafa]', gridBorder: 'border-zinc-200', checkBg: 'bg-emerald-50', checkFill: 'bg-emerald-600', scrollThumb: '#d4d4d8', scrollTrack: 'transparent', heatmapEmpty: 'bg-zinc-200' };
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (metaSnap.exists()) {
            ...
            const now = new Date();
            await setDoc(metaRef, { joined: now.toISOString(), tutorialSeen: false }, { merge: true });
            setJoinedDate(now);
  ...
  useEffect(() => {
    ...
    const unsubMeta = onSnapshot(doc(db, 'users', user.uid, 'data', 'metadata'), (doc) => {
        if (doc.exists() && doc.data().visionBoard) setVisionBoardImg(doc.data().visionBoard);
    });
  ...
  const finishTutorial = async () => {
      ...
      if (user) {
          try { await setDoc(doc(db, 'users', user.uid, 'data', 'metadata'), { tutorialSeen: true }, { merge: true }); } catch (e) {}
      }
  };
  ...
  const handleImageUpload = (event) => {
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            if (user) {
                setDoc(doc(db, 'users', user.uid, 'data', 'metadata'), { visionBoard: compressedDataUrl }, { merge: true });
            }
        };
    };
  };
  ...
  const saveChecksToCloud = async (newChecks) => {
    if (!user) return;
    setChecks(newChecks);
    try { await setDoc(doc(db, 'users', user.uid, 'data', 'checks'), newChecks); } catch (e) {}
  };
  ...
  const handleLogin = async () => {
    ...
    try { await signInWithPopup(auth, provider); } catch (error) { if (error.code === 'auth/unauthorized-domain') alert(`Login Blocked: Add "${window.location.hostname}" to Firebase.`); }
  };
  ...
  const toggleCheck = (habitId, day, event) => {
    ...
    const isChecking = !newChecks[key];

    if (isChecking) {
        const xpAmount = 25; 
        const newPopup = { id: Date.now(), x: event.clientX, y: event.clientY, amount: xpAmount };
  ...
  const addManualHabit = () => {
    ...
    const exists = currentHabits.some(h => h.name.trim().toLowerCase() === normalizedName);
    if (exists) return alert("Habit already exists.");

    const newHabit = { id: Date.now(), name: manualHabitName, icon: manualHabitIcon };
    const newMonthHabits = { ...monthlyHabits, [currentMonthKey]: [...(monthlyHabits[currentMonthKey] || []), newHabit] };
    saveHabitsToCloud(newMonthHabits);
    setManualHabitName("");
  ...
  const copyFromPreviousMonth = () => {
    ...
    const newMonthHabits = { ...monthlyHabits, [currentMonthKey]: [...(monthlyHabits[currentMonthKey] || []), ...prevHabits] };
    saveHabitsToCloud(newMonthHabits);
  };

  const { heatmapData, currentStreak, activeDays, monthsLabels, calculatedXP } = useMemo(() => {
    const data = [];
    let streak = 0;
    let active = 0;
    let totalXP = 0;
    ...
    for (let i = 364; i >= 0; i--) {
      ...
      if (day === 1) labels[colIndex] = d.toLocaleString('default', { month: 'short' });
      
      const monthKey = `${year}-${month}`;
      const habitsForMonth = monthlyHabits[monthKey] || [];
      const totalForDay = habitsForMonth.length;
      
      let count = 0;
      habitsForMonth.forEach(h => {
        if (checks[`${h.id}-${year}-${month}-${day}`]) count++;
      });
      
      if (count > 0) {
          active++;
          const dayXP = Math.min(count * xpPerCheck, maxDailyXP);
          totalXP += dayXP;
      }
      
      const isPerfect = totalForDay > 0 && count === totalForDay;
      data.push({ date: d, count, isPerfect });
    }

    for (let i = 363; i >= 0; i--) {
        if (data[i].isPerfect) streak++; else break;
    }
    if (data[364].isPerfect) streak++;

    return { heatmapData: data, currentStreak: streak, activeDays: active, monthsLabels: labels, calculatedXP: totalXP };
  }, [checks, monthlyHabits]);
  ...
  const stats = useMemo(() => {
    ...
    const habitStats = currentHabits.map(h => {
      ...
      totalChecks += count;
      return { ...h, count, percent: totalPossible > 0 ? (count / totalDaysShown) * 100 : 0 };
    });
    const dailyStats = Array.from({ length: totalDaysShown }).map((_, i) => {
        const day = i + startDay;
        let completedToday = 0;
        currentHabits.forEach(h => { if (checks[`${h.id}-${currentYear}-${currentMonth}-${day}`]) completedToday++; });
        return { day, count: completedToday, percent: currentHabits.length > 0 ? (completedToday / currentHabits.length) * 100 : 0 };
    });
  ...
  const renderTutorial = () => {
      ...
      return (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm transition-all duration-500" onClick={() => finishTutorial()}>
             <div className={`max-w-md w-full ${theme.card} border ${theme.border} p-8 rounded-lg shadow-2xl text-center animate-in fade-in zoom-in duration-300 relative`} onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={() => finishTutorial()} className={`absolute top-4 right-4 ${theme.textMuted} hover:text-white transition-colors`}><X size={20} /></button>
                ...
                <div className="flex gap-4 justify-center items-center">
                   <button 
                     ...
                     className={`p-3 rounded-full border ${theme.border} transition-colors flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed ${tutorialStep > 0 ? 'hover:bg-zinc-800 text-white' : 'text-zinc-600'}`}
                   >
                     <ChevronLeft size={20} />
                   </button>
                   ...
                   <div className="flex gap-2">
                        {TUTORIAL_STEPS.map((_, i) => (
                            <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${i === tutorialStep ? 'bg-emerald-500 w-3' : 'bg-zinc-700'}`} />
                        ))}
                   </div>

                   <button 
                     onClick={() => {
                         if (!isLastStep) setTutorialStep(s => s + 1);
                     }}
                     disabled={isLastStep}
                     className={`p-3 rounded-full border ${theme.border} transition-colors flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed ${!isLastStep ? 'hover:bg-zinc-800 text-white' : 'text-zinc-600'}`}
                   >
    ...
    return (
      <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans p-4 md:p-12 flex flex-col items-center justify-center`}>
        <style>{`
          ...
          .custom-scrollbar::-webkit-scrollbar-thumb { background-color: ${theme.scrollThumb}; border-radius: 99px; }
        ...
        <div className="w-full max-w-6xl mb-8 flex items-center justify-between">
           <div>
             <button onClick={() => setViewMode('app')} className={`flex items-center gap-2 text-sm ${theme.textMuted} hover:text-white transition-colors mb-2`}>
           ...
           <div className="flex items-center gap-3">
              <button onClick={() => setDarkMode(!darkMode)} className={`p-3 rounded-full border ${theme.border} ${theme.hover} transition-all`}>
              ...
              <div className={`w-10 h-10 rounded-full border ${theme.border} overflow-hidden bg-black flex items-center justify-center text-sm font-bold ${theme.text}`}>
                 {user?.photoURL && !imgError ? <img src={user.photoURL} alt="Profile" className="w-full h-full object-cover" onError={() => setImgError(true)} /> : (user?.displayName ? user.displayName[0].toUpperCase() : <User size={18} />)}
              </div>
           </div>
        </div>
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
           <div className={`md:col-span-1 ${theme.card} border ${theme.border} rounded-lg p-8 flex flex-col items-center justify-between text-center shadow-sm`}>
              <div>
                <div className={`w-32 h-32 rounded-full border-4 ${theme.border} overflow-hidden mb-6 flex items-center justify-center bg-black text-4xl font-serif mx-auto`}>
                   {user?.photoURL && !imgError ? <img src={user.photoURL} alt="Profile" className="w-full h-full object-cover" onError={() => setImgError(true)} /> : (user?.displayName ? user.displayName[0].toUpperCase() : <User size={40} />)}
                </div>
                <h2 className="text-2xl font-serif">{user?.displayName || 'User'}</h2>
                <p className={`text-sm ${theme.textMuted} mt-2`}>Level {currentLevel}</p>
                <div className="mt-6">
                   <div className="flex justify-between text-xs font-mono mb-2 text-emerald-400"><span>{calculatedXP} XP</span><span>{nextLevelXP} XP</span></div>
           ...
           <div className="md:col-span-2 flex flex-col gap-6">
              <div className={`flex-1 ${theme.card} border ${theme.border} rounded-lg p-8 overflow-hidden flex flex-col relative group min-h-[300px]`}>
                 ...
                 <div 
                    className="flex-1 mt-4 rounded-lg bg-black/50 relative overflow-hidden flex items-center justify-center border border-zinc-800 min-h-[200px]"
              ...
              <div className={`${theme.card} border ${theme.border} rounded-lg p-8 overflow-hidden`}>
                 ...
                 <div className={`flex items-center gap-2 mt-4 text-[10px] font-mono ${theme.textMuted}`}><span>Less</span><div className={`w-2.5 h-2.5 rounded-[2px] ${theme.heatmapEmpty}`} /><div className="w-2.5 h-2.5 rounded-[2px] bg-emerald-900" /><div className="w-2.5 h-2.5 rounded-[2px] bg-emerald-600" /><div className="w-2.5 h-2.5 rounded-[2px] bg-emerald-400" /><span>More</span></div>
              </div>
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans p-4 md:p-12 flex flex-col items-center justify-center relative transition-colors duration-500 selection:bg-emerald-500/20 animate-in fade-in duration-700`}>
      <style>{`
        ...
        @keyframes floatUp { 0% { opacity: 1; transform: translateY(0) scale(1); } 100% { opacity: 0; transform: translateY(-20px) scale(1.1); } }
      `}</style>

      {renderTutorial()}

      {xpPopups.map(p => (
        <div key={p.id} className="fixed pointer-events-none text-emerald-400 font-mono font-bold text-sm z-[60]" style={{ left: p.x + 10, top: p.y - 20, animation: 'floatUp 0.8s ease-out forwards' }}>+{p.amount} XP</div>
      ))}
        ...
        <div className="w-full md:w-auto">
           <div className="flex items-center gap-6 mb-2">
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => navigateMonth(1)} className={`p-2 rounded-md transition-all ${theme.hover}`}><ChevronRight className="w-5 h-5" /></button>
              </div>
              <h1 className="text-3xl md:text-5xl font-serif font-light tracking-tight">
                {currentDate.toLocaleString('default', { month: 'long' })}
                <span className={`text-lg font-mono ml-3 opacity-40 font-normal align-top`}>{currentYear}</span>
              </h1>
           </div>
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          ...
          >
             {user?.photoURL && !imgError ? (
               ...
               <div className={`w-full h-full ${theme.cardLight} flex items-center justify-center text-sm font-bold ${theme.text}`}>
      ...
      <div className={`w-full max-w-7xl mb-6 flex gap-6 border-b ${theme.tabBorder}`}>
        ...
        <button onClick={() => setActiveTab('analysis')} className={`pb-2 text-sm font-mono tracking-wider uppercase transition-colors border-b-2 ${activeTab === 'analysis' ? `border-emerald-500 ${theme.text}` : `border-transparent ${theme.textMuted} hover:text-zinc-300`}`}>// Analytics</button>
      </div>
        ...
        <div className={`w-full max-w-7xl border-y ${theme.border} md:border ${theme.card} md:rounded-lg overflow-hidden flex flex-col h-fit shadow-sm animate-in fade-in duration-500`}>
          <div className="flex-1 flex flex-col min-w-0">
             <div className={`p-4 border-b ${theme.border} flex justify-between items-center ${theme.bg}`}>
                ...
                {!isPastMonth && (
                  <button onClick={() => setShowAddModal(true)} className={`flex items-center gap-2 px-3 py-1.5 text-xs font-mono border ${theme.border} rounded hover:border-emerald-500 transition-colors`}>
                   ...
                   <div className="flex flex-col items-center justify-center p-12 text-center h-96">
                      ...
                      {!isPastMonth && (
                        <div className="flex flex-col sm:flex-row gap-4">
                          <button onClick={() => setShowAddModal(true)} className={`px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium tracking-wide rounded-sm shadow-lg shadow-emerald-900/20`}>Start New Habit</button>
                    ...
                    <table className="min-w-full border-collapse">
                      ...
                      <tbody>
                        {currentHabits.map((habit) => (
                          <tr key={habit.id} className={`group ${theme.hover} transition-colors`}>
                            <td className={`sticky left-0 z-10 ${theme.gridHeader} group-hover:${theme.hover} p-4 border-r ${theme.border} border-b ${theme.border} transition-colors`}>
                              <div className="flex justify-between items-center group/cell">
                                  <div className="flex items-center gap-3"><span className="text-xl">{habit.icon}</span><span className="font-medium text-sm tracking-wide">{habit.name}</span></div>
                                  {!isPastMonth && <button onClick={(e) => deleteHabit(habit.id, e)} className={`opacity-0 group-hover/cell:opacity-100 p-1.5 hover:text-red-500 transition-all`}><X className="w-3 h-3" /></button>}
                              </div>
                            </td>
                            {Array.from({ length: totalDaysShown }).map((_, dIdx) => {
                              const dayNum = dIdx + startDay;
                              const isChecked = !!checks[`${habit.id}-${currentYear}-${currentMonth}-${dayNum}`];
                              const isToday = dayNum === new Date().getDate() && currentMonth === new Date().getMonth() && currentYear === new Date().getFullYear();
                              const isLocked = (strictMode && !isToday) || isPastMonth;
                              return (
                                <td 
                                  key={dIdx} 
                                  ...
                                  onClick={(e) => !isLocked && toggleCheck(habit.id, dayNum, e)}
                                >
                                  <div className="w-full h-14 flex items-center justify-center">{isChecked ? <div className={`w-5 h-5 rounded-sm ${theme.checkFill} flex items-center justify-center shadow-sm`}><Check className="w-3.5 h-3.5 text-white" strokeWidth={3} /></div> : <div className={`w-1.5 h-1.5 rounded-full ${theme.gridBorder} bg-current opacity-10 group-hover:opacity-20`} />}</div>
                                </td>
                              );
                            })}
                          </tr>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className={`lg:col-span-1 p-8 rounded-lg border ${theme.border} ${theme.cardLight} flex flex-col items-center justify-center relative overflow-hidden h-80`}>
                 <div className="relative z-10 flex flex-col items-center">
                    <p className={`text-xs font-mono uppercase tracking-widest ${theme.textMuted} mb-6`}>Month Score</p>
                    <div className="relative w-48 h-48">
                        <svg className="w-full h-full transform -rotate-90">
                          <circle cx="96" cy="96" r="60" stroke="currentColor" strokeWidth="12" fill="transparent" className="opacity-10" />
                          <circle cx="96" cy="96" r="60" stroke={darkMode ? '#34d399' : '#059669'} strokeWidth="12" fill="transparent" strokeDasharray={2 * Math.PI * 60} strokeDashoffset={(2 * Math.PI * 60) - (stats.overallPercent / 100) * (2 * Math.PI * 60)} strokeLinecap="butt" className="transition-all duration-1000 ease-out" />
                        </svg>
                        <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-5xl font-serif font-light">{stats.overallPercent.toFixed(0)}%</span></div>
                     </div>
                 </div>
                     ...
                     <div className="flex items-end gap-1 h-40">
                        {stats.dailyStats.map((d, i) => (
                           <div key={i} className="flex-1 h-full flex items-end group relative">
                              <div className="absolute bottom-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black text-white text-[10px] py-1 px-2 rounded whitespace-nowrap z-10 pointer-events-none left-1/2 -translate-x-1/2">Day {d.day}: {d.count}</div>
                              <div style={{ height: `${Math.max(d.percent, 5)}%` }} className={`w-full ${d.percent >= 100 ? 'bg-emerald-500' : d.percent > 50 ? 'bg-emerald-500/60' : d.percent > 0 ? 'bg-emerald-500/30' : 'bg-zinc-500/10'} min-h-[4px] rounded-t-[1px] transition-all hover:bg-emerald-400`} />
                           </div>
                        ))}
                     </div>
                 </div>
                 <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {stats.habitStats.map(stat => (
                           <div key={stat.id} className={`p-4 rounded border ${theme.border} ${theme.cardLight} flex items-center justify-between`}>
                              <div className="flex items-center gap-3"><span className="text-xl">{stat.icon}</span><span className="font-medium text-sm">{stat.name}</span></div>
                              <div className="flex items-center gap-3"><div className={`w-16 h-1 ${theme.emptyBar} rounded-full overflow-hidden`}><div className={`h-full ${theme.accentBg}`} style={{ width: `${stat.percent}%` }} /></div><span className="font-mono text-xs opacity-60">{stat.percent.toFixed(0)}%</span></div>
                           </div>
                        ))}
                    </div>
                 </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`${theme.card} border ${theme.border} w-full max-w-md p-8 rounded-lg shadow-2xl`}>
              <div className="flex justify-between items-start mb-8">
                <div><h3 className="text-2xl font-serif">New Entry</h3><p className={`text-sm ${theme.textMuted} mt-1`}>Add a routine for {currentDate.toLocaleString('default', { month: 'long' })}.</p></div>
              ...
              <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input type="text" value={manualHabitIcon} onChange={(e) => setManualHabitIcon(e.target.value)} className={`w-12 text-center bg-transparent border-b ${theme.border} py-1 text-lg focus:outline-none focus:border-emerald-500 transition-colors`} maxLength={2} />
                    </div>
                  </div>
                  <button onClick={addManualHabit} disabled={!manualHabitName.trim()} className={`w-full py-4 mt-4 bg-emerald-600 hover:bg-emerald-700 text-white font-medium tracking-wide rounded-sm disabled:opacity-50 transition-all`}>Confirm Addition</button>
              </div>
          </div>
        </div>
      )}
